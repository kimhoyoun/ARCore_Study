...
//        mColorCorrection[3] = colorCorrection[3];
    ...
    float[][] getMinMaxPoint(){
        ...
        // 위값과 좌표를 통해서 실제 obj의 좌표를 계산한 값을 얻을 수 있다.

        float[] minPoint = new float[4];
        Matrix.multiplyMV(minPoint, 0, mModelMatrix, 0,
                new float[]{myMinPoint[0],myMinPoint[1],myMinPoint[2],1.0f},0);

        float[] maxPoint = new float[4];
        Matrix.multiplyMV(maxPoint, 0, mModelMatrix, 0,
                new float[]{myMaxPoint[0],myMaxPoint[1],myMaxPoint[2],1.0f},0);

        float [] res0 = new float[3];
        float [] res1 = new float[3];
        // 계산된 좌표가 아님!!
//        res[0] = Math.min(myMinPoint[0], myMaxPoint[0]);
//        res[1] = Math.min(myMinPoint[1], myMaxPoint[1]);
//        res[2] = Math.min(myMinPoint[2], myMaxPoint[2]);
    ...
    void calMyMinMax(){
        ...
        float[] vertices = ObjData.getNormalsArray(mObj);

        // x,y,z를 가지고 있는 점들의 배열
        // getNumVertices() 점의 갯수 ex) 아래 : 3
        // 점1, 점2, 점3, 점4
        // vertices : (점1.x, 점1.y, 점1.z, 점2.x, 점2.y, 점2.z, 점3.x, 점3.y, 점3.z) // 일열로 나열돼있음.
        // vertices의 0, 1, 2번째는 다른걸 줘서 i를 1번부터 시작함.
        for(int i =1; i<mObj.getNumVertices(); i++){
            ...
            myMaxPoint[0] = Math.max(myMinPoint[0], vertices[i*3+0]); // x
            myMaxPoint[1] = Math.max(myMinPoint[1], vertices[i*3+1]); // y
            myMaxPoint[2] = Math.max(myMinPoint[2], vertices[i*3+2]); // z
        }
    }
}
